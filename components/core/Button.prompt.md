...
```jsx
<Button title="Começar treino  →" color="var(--forge-accent)" onClick={start} />
```

Variants: `size="sm"` (36px, for inline header actions like "Ajustar"); `disabled` (0.5 opacity); pass any module accent as `color` (e.g. `var(--forge-nutrition)` for the Nutrição module) — text contrast is derived automatically via `onColor()`.
