  ...
  return (
    <div style={{ padding: "8px 20px 24px", maxWidth: 480, margin: "0 auto", boxSizing: "border-box" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 4 }}>
        <div>
          <div style={{ fontFamily: "var(--forge-font-body)", fontWeight: 700, fontSize: 12, letterSpacing: 2, textTransform: "uppercase", color: "var(--forge-text-faint)", marginBottom: 4 }}>Treino</div>
      ...
      <div style={{ fontSize: 12, letterSpacing: 2, textTransform: "uppercase", color: "var(--forge-text-faint)", fontFamily: "var(--forge-font-body)", fontWeight: 700, marginTop: 4, marginBottom: 14 }}>
        Terça · treino de hoje
      </div>

      {/* today card */}
      <div style={{ display: "flex", backgroundColor: "var(--forge-surface)", border: "1px solid var(--forge-border)", borderRadius: "var(--forge-radius-card)", overflow: "hidden", marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div style={{ flex: 1, minWidth: 0, paddingRight: 10 }}>
              <div style={{ display: "inline-block", backgroundColor: workout.accent, borderRadius: 6, padding: "3px 11px" }}>
                <span style={{ color: "var(--forge-on-accent)", fontFamily: "var(--forge-font-body)", fontWeight: 800, fontSize: 13, letterSpacing: 1 }}>{workout.tag}</span>
              </div>
              <div style={{ fontFamily: "var(--forge-font-title)", fontSize: 34, lineHeight: "36px", marginTop: 12, textTransform: "uppercase", color: "var(--forge-text)" }}>{workout.name}</div>
              <div style={{ color: "#8a8a92", fontSize: 14, marginTop: 6, fontFamily: "var(--forge-font-body)" }}>{workout.items} exercícios · {setsDone}/{setsTotal} séries</div>
            </div>
            <Ring size={100} stroke={11} progress={pct} color={workout.accent}>
              <span style={{ fontSize: 24, fontFamily: "var(--forge-font-title)", color: "var(--forge-text)" }}>{Math.round(pct * 100)}<span style={{ fontSize: 13 }}>%</span></span>
            </Ring>
          </div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, border: "1px dashed #3a3a42", borderRadius: 12, padding: 13, marginBottom: 28, cursor: "pointer" }}>
        ...
        <span style={{ color: "#b0b0b8", fontSize: 14, fontFamily: "var(--forge-font-body)", fontWeight: 700 }}>Fazer outro treino hoje</span>
      </div>

      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <span style={{ fontSize: 12, letterSpacing: 2, textTransform: "uppercase", color: "var(--forge-text-faint)", fontFamily: "var(--forge-font-body)", fontWeight: 700 }}>Sua semana</span>
        <div onClick={onEditWorkout} style={{ display: "flex", alignItems: "center", gap: 6, border: "1px solid var(--forge-border-input)", borderRadius: 8, padding: "6px 12px", cursor: "pointer" }}>
          ...
          <span style={{ color: "#b0b0b8", fontSize: 12, fontFamily: "var(--forge-font-body)", fontWeight: 700 }}>Editar agenda</span>
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 9, marginBottom: 24 }}>
        {week.map((w) => (
          <div key={w.day} style={{ display: "flex", alignItems: "center", gap: 12, backgroundColor: "var(--forge-surface)", border: `1px solid ${w.today ? workout.accent : "var(--forge-border)"}`, borderRadius: 12, padding: 12, opacity: w.today ? 1 : 0.72 }}>
            <span style={{ width: 40, fontSize: 11, fontFamily: "var(--forge-font-body)", fontWeight: 800, textTransform: "uppercase", color: w.today ? "var(--forge-text)" : "var(--forge-text-faint)" }}>{w.day}</span>
            {w.tag ? (
              <>
                <div style={{ width: 34, height: 34, borderRadius: 8, backgroundColor: workout.accent, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <span style={{ color: "var(--forge-on-accent)", fontFamily: "var(--forge-font-body)", fontWeight: 800, fontSize: 13 }}>{w.tag}</span>
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ color: "var(--forge-text)", fontFamily: "var(--forge-font-body)", fontWeight: 600, fontSize: 15 }}>{w.name}</div>
                  <div style={{ color: "var(--forge-text-dim)", fontSize: 12, fontFamily: "var(--forge-font-body)" }}>{w.today ? "Hoje " : ""}{w.exos} exercícios</div>
                </div>
              </>
            ) : (
              <>
                ...
                <span style={{ flex: 1, color: "var(--forge-text-faint)", fontFamily: "var(--forge-font-body)", fontWeight: 600, fontSize: 15 }}>Descanso</span>
              </>
            )}
          </div>
        ))}
