  ...
  return (
    <FullScreen visible={visible} onClose={onClose} title="Montar treino" footer={<Button title="Salvar treino" onClick={onClose} style={{ width: "100%" }} />}>
      ...
      {exercises.map((ex, i) => (
        ...
        >
          <span style={{ width: 22, color: "var(--forge-text-dimmer)", fontFamily: "var(--forge-font-title)", fontSize: 16 }}>{i + 1}</span>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ color: "var(--forge-text)", fontFamily: "var(--forge-font-body)", fontWeight: 600, fontSize: 13.5 }}>{ex.name}</div>
      ...
      >
        ...
        <span style={{ color: "var(--forge-accent)", fontSize: 13, fontFamily: "var(--forge-font-body)", fontWeight: 700 }}>Adicionar exercício</span>
      </div>
    </FullScreen>
  );
}
